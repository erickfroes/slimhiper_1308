...
import type { AppointmentStatus, PatientStatus, FinancialStatus, PackageStatus, AdherenceLevel } from '@/domain/types';

type BadgeVariant = AppointmentStatus | PatientStatus | FinancialStatus | PackageStatus | AdherenceLevel | 'pago' | 'pendente' | 'vencido' | 'cancelado' | 'em_analise' | 'pendente_assinatura' | 'assinado';
...
const badgeConfig: Record<string, { label: string; classes: string }> = {
  ...
  aguardando_medico: { label: 'Aguard. Médico', classes: 'bg-indigo-50 text-indigo-700 border-indigo-200' },
  ...
  // Document statuses
  pendente_assinatura: { label: 'Pend. Assinatura', classes: 'bg-amber-50 text-amber-700 border-amber-200' },
...
export default function StatusBadge({ status, size = 'sm', dot = false }: StatusBadgeProps) {
  const config = badgeConfig[status] ?? { label: status, classes: 'bg-slate-100 text-slate-600 border-slate-200' };
  ...
  return (
    <span className={['inline-flex items-center gap-1 rounded-full border font-medium', config.classes, sizeClasses[size]].join(' ')}>
