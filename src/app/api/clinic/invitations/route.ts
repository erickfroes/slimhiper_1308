...
export async function POST(request: Request) {
  ...
  if (!session.canManageTenantUsers()) {
    return jsonError('Sem permissao para convidar membros da clinica.', 403);
  }

  const tenantId = session.activeTenant?.id ?? '';
  if (!isUuid(tenantId)) {
    return jsonError('Tenant ativo invalido.', 400);
  }
  ...
  if (roleError) return jsonError('Falha ao validar papel do tenant.', 500);
  if (!role) return jsonError('Papel nao configurado para este tenant.', 400);
  ...
  if (unitId) {
    ...
    if (unitError) return jsonError('Falha ao validar unidade.', 500);
    if (!unit) return jsonError('Unidade nao encontrada para este tenant.', 404);
  }
  ...
  try {
    ...
    const { error: auditError } = await admin.from('audit_logs').insert({
      ...
      action: 'clinic_tenant_membership.invited',
