...
import { Scale, Target, Activity, CheckSquare, Clock, Calendar, FileText, DollarSign, Stethoscope, TrendingUp, MessageCircle, ChevronRight, Pen, Bell, ClipboardList, CreditCard,  } from 'lucide-react';
...
function SectionHeader({ icon: Icon, label, iconBg, iconColor, badge }: {
  ...
  return (
    <div className="flex items-center gap-2 mb-3">
      <div className={['w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0', iconBg].join(' ')}>
...
export default function TabResumo({ data }: TabResumoProps) {
  const { clinicalStatus, alerts, tasks, upcomingAppointments, financial, activePackage, documents, chat } = data;
  ...
  const bmiCategory =
    clinicalStatus.currentBmi < 18.5 ? 'Abaixo do peso'
      : clinicalStatus.currentBmi < 25 ? 'Normal'
      : clinicalStatus.currentBmi < 30 ? 'Sobrepeso' : 'Obesidade';
          ...
          <div className="card-base p-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4">
              <div className="bg-muted/50 rounded-xl p-3">
                ...
                <p className="text-base font-bold text-foreground tabular-nums">{clinicalStatus.startWeightKg} kg</p>
              </div>
              <div className="bg-muted/50 rounded-xl p-3">
                <p className="text-xs text-muted-foreground mb-0.5">Peso atual</p>
                <p className="text-base font-bold text-primary tabular-nums">{clinicalStatus.currentWeightKg} kg</p>
              </div>
              <div className="bg-muted/50 rounded-xl p-3">
                <p className="text-xs text-muted-foreground mb-0.5">Meta</p>
                <p className="text-base font-bold text-foreground tabular-nums">{clinicalStatus.goalWeightKg} kg</p>
              </div>
            </div>
            ...
            <p className="text-xs text-muted-foreground mt-2">
              Última medição: {clinicalStatus.lastMeasuredAt} · Progresso: {clinicalStatus.progressPercent}% da meta · Perdeu {clinicalStatus.weightLostKg} kg
            </p>
          </div>
            ...
            {recentAppointments.length === 0 ? (
              ...
              <div className="space-y-2">
                {recentAppointments.map((appt) => (
                  <div key={appt.id} className="flex items-start gap-3 p-3 rounded-xl border border-border hover:bg-muted/40 transition-colors">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-2 flex-wrap">
                        <p className="text-sm font-semibold text-foreground">{apptTypeLabel[appt.type] ?? appt.type}</p>
                        <StatusBadge status={appt.status} size="xs" />
                      </div>
                      <p className="text-xs text-muted-foreground mt-0.5">{appt.professionalName} · {appt.professionalRole}</p>
                ...
                {documents.slice(0, 5).map((doc) => (
                  <div key={doc.id} className="flex items-center gap-3 p-3 rounded-xl border border-border hover:bg-muted/40 transition-colors">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground">{docTypeLabel[doc.type] ?? doc.type} · {doc.createdAt}</p>
                    </div>
                    <span className={['text-xs font-medium px-2 py-0.5 rounded-full border flex-shrink-0', docStatusColor[doc.status] ?? 'text-slate-500 bg-slate-50 border-slate-200'].join(' ')}>
        ...
        <div className="space-y-4">
            ...
            {nextAppointments.length === 0 ? (
              ...
              <div className="space-y-2">
                {nextAppointments.map((appt) => (
                  <div key={appt.id} className="p-3 rounded-xl border border-border hover:bg-muted/40 transition-colors">
                    ...
                    <p className="text-xs text-muted-foreground">{apptTypeLabel[appt.type] ?? appt.type}</p>
            ...
            {docsAwaitingSignature.length === 0 ? (
              ...
              <div className="space-y-2">
                {docsAwaitingSignature.map((doc) => (
                  <div key={doc.id} className="flex items-center gap-2 p-3 rounded-xl border border-amber-200 bg-amber-50">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[10px] text-amber-600">{docTypeLabel[doc.type]} · {doc.createdAt}</p>
                    </div>
            ...
            <div className="space-y-2">
              ...
              {financial.nextDueDate && (
                <div className="flex items-center justify-between p-3 rounded-xl border border-amber-200 bg-amber-50">
                  ...
                  <span className="text-xs font-semibold text-amber-800">{financial.nextDueDate}</span>
                </div>
              )}
          ...
          <div className="card-base p-4">
            <SectionHeader
              ...
              iconColor={chat?.unreadCount && chat.unreadCount > 0 ? 'text-red-500' : 'text-slate-500'}
              badge={
            ...
            {!chat ? (
              ...
              <div className={['p-3 rounded-xl border', chat.unreadCount > 0 ? 'border-red-200 bg-red-50' : 'border-border bg-muted/30'].join(' ')}>
                <p className={['text-xs font-medium mb-1', chat.unreadCount > 0 ? 'text-red-700' : 'text-foreground'].join(' ')}>
                  {chat.lastMessageFrom}
                </p>
                <p className="text-xs text-muted-foreground line-clamp-2 italic">"{chat.lastMessagePreview}"</p>
                <p className="text-[10px] text-muted-foreground mt-1.5">
                  {new Date(chat.lastMessageAt).toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit' })}
                  {' às '}
