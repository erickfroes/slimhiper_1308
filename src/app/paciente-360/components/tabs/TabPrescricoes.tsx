...
import { Plus, FileText, Send, XCircle, Copy, ChevronDown, ChevronUp, Stethoscope, Leaf, Salad, BookOpen, ShieldAlert, CheckCircle2, Clock, AlertCircle, FileSignature,  } from 'lucide-react';
...
type PrescCategory = 'prescricao_medica' | 'suplementacao' | 'orientacoes_nutricionais' | 'orientacoes_gerais';
...
function StatusBadge({ status }: { status?: string }) {
  const map: Record<string, { label: string; cls: string; icon: React.ElementType }> = {
    ...
    pendente_assinatura: { label: 'Pend. Assinatura', cls: 'bg-amber-50 text-amber-700 border-amber-200', icon: Clock },
    rascunho: { label: 'Rascunho', cls: 'bg-slate-100 text-slate-500 border-slate-200', icon: FileText },
  };
  const cfg = map[status ?? ''] ?? { label: status ?? '—', cls: 'bg-gray-100 text-gray-500 border-gray-200', icon: AlertCircle };
  const Icon = cfg.icon;
  return (
...
function SignatureBadge({ sig }: { sig?: string }) {
  if (!sig || sig === 'nao_requerido') return <span className="text-xs text-muted-foreground">—</span>;
  if (sig === 'assinado') return <span className="inline-flex items-center gap-1 text-xs text-emerald-700"><CheckCircle2 size={11} />Assinado</span>;
  return <span className="inline-flex items-center gap-1 text-xs text-amber-600"><Clock size={11} />Pendente</span>;
}

// ─── Prescription Card ────────────────────────────────────────────────────────

function PrescriptionCard({ presc, catConfig }: { presc: PatientPrescriptionSummary; catConfig: CategoryConfig }) {
  ...
  return (
    <div className="card-base overflow-hidden">
      ...
      <div className="p-4 flex items-start gap-3">
        <div className={`w-9 h-9 rounded-xl ${catConfig.bgColor} flex items-center justify-center flex-shrink-0`}>
        ...
        <div className="flex-1 min-w-0">
          ...
          {presc.dosage !== '—' && (
            <p className="text-xs text-muted-foreground mt-0.5">{presc.dosage} · {presc.frequency}</p>
          )}
        </div>
        <button
      ...
      {expanded && (
        <div className="border-t border-border px-4 py-3 bg-muted/30 space-y-3">
          ...
          {presc.notes && (
            <p className="text-xs text-muted-foreground italic border-t border-border pt-2">{presc.notes}</p>
          )}
          {/* Per-card actions */}
          <div className="flex flex-wrap gap-2 border-t border-border pt-2">
            ...
            <button className="inline-flex items-center gap-1.5 text-xs font-medium px-2.5 py-1.5 rounded-lg border border-border hover:bg-muted transition-colors">
              <Send size={12} />Enviar para assinatura
            </button>
            <button className="inline-flex items-center gap-1.5 text-xs font-medium px-2.5 py-1.5 rounded-lg border border-border hover:bg-muted transition-colors">
    ...
    <div className="space-y-3">
      ...
      <div className={`flex items-center justify-between px-3 py-2 rounded-xl ${config.bgColor} border ${config.borderColor}`}>
        <div className="flex items-center gap-2">
          ...
          <span className={`text-xs font-medium px-1.5 py-0.5 rounded-full ${config.bgColor} ${config.color} border ${config.borderColor}`}>
            {items.length}
          </span>
        </div>
      </div>
      ...
      {isRestricted ? (
        <div className="card-base p-5 flex items-start gap-3 border-amber-200 bg-amber-50/40">
          ...
          <div>
            <p className="text-sm font-semibold text-amber-800">Acesso restrito ao escopo profissional</p>
            <p className="text-xs text-amber-700 mt-0.5">
              Prescrições médicas de medicamentos são de responsabilidade exclusiva do médico. Como nutricionista, você não tem permissão para visualizar, criar ou editar este tipo de prescrição.
            </p>
          </div>
        </div>
      ) : items.length === 0 ? (
...
export default function TabPrescricoes({ prescriptions, currentRole = 'physician' }: TabPrescricoesProps) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <p className="text-sm font-semibold text-foreground">
          ...
          <span className="ml-2 text-xs font-normal text-muted-foreground">({prescriptions.length} registros)</span>
        </p>
        <div className="flex flex-wrap gap-2">
          ...
          <button className="inline-flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-lg border border-border bg-background hover:bg-muted transition-colors">
            <FileSignature size={13} />Nova orientação
          </button>
        </div>
      </div>
