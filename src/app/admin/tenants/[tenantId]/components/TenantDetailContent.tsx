...
const mockTenantDetails: Record<string, TenantDetail> = {
  ...
  T004: {
    ...
    website: 'metabolichealth.com.br',
...
const mockUsers: TenantUser[] = [
  {
    ...
    role: 'Administrador',
    status: 'active',
    lastLogin: '2026-05-07 19:30',
    mfaEnabled: true,
  },
  {
    id: 'U002',
    name: 'Dra. Fernanda Lima',
    email: 'fernanda@corposaude.com.br',
    role: 'Médico',
    status: 'active',
    lastLogin: '2026-05-07 18:45',
    mfaEnabled: true,
  },
  {
    id: 'U003',
    name: 'Nutr. Juliana Pires',
    email: 'juliana@corposaude.com.br',
    role: 'Nutricionista',
    status: 'active',
    lastLogin: '2026-05-07 17:00',
    mfaEnabled: false,
  },
  {
    id: 'U004',
    name: 'Carlos Mendes',
    email: 'carlos@corposaude.com.br',
    role: 'Recepcionista',
    status: 'active',
  ...
  },
  {
    id: 'U005',
    name: 'Patrícia Souza',
    email: 'patricia@corposaude.com.br',
    role: 'Coordenador',
    status: 'inactive',
    lastLogin: '2026-04-30 10:00',
    mfaEnabled: false,
  },
];
...
const mockAuditLogs: AuditEntry[] = [
  {
    id: 'A001',
    action: 'plan_changed',
    description: 'Plano migrado de Professional para Enterprise',
    admin: 'Admin Carlos',
  ...
  },
  {
    ...
    description: 'Feature "Builder de Programas" habilitada',
    admin: 'Admin Maria',
  ...
  },
  {
    ...
    description: 'Sessão de suporte iniciada por Admin Carlos',
    admin: 'Admin Carlos',
  ...
  },
  {
    ...
    description: 'Integração Asaas reconectada com sucesso',
    admin: 'Admin Maria',
  ...
  },
  {
    ...
    description: 'Fatura de Maio/2026 gerada — R$ 1.490,00',
    admin: 'Admin Sistema',
  ...
  },
  {
    ...
    description: 'Revisão de segurança: IP suspeito bloqueado',
    admin: 'Admin Carlos',
    timestamp: '2026-04-28 20:10',
    category: 'security',
  },
];
...
const mockSupportSessions: SupportSession[] = [
  {
    ...
    status: 'open',
    priority: 'alto',
    subject: 'Integração Asaas com falha intermitente',
    assignedTo: 'Admin Carlos',
    openedAt: '2026-05-07 14:00',
  ...
  },
  {
    ...
    openedAt: '2026-05-03 10:00',
    lastActivity: '2026-05-04 09:30',
  },
];
...
const mockBreakGlass: BreakGlassRequest[] = [
  {
    ...
    reason: 'Investigar falha crítica de integração reportada pelo cliente',
    status: 'approved',
    requestedAt: '2026-05-07 14:30',
    approvedBy: 'Admin Maria',
    expiresAt: '2026-05-07 16:30',
    scope: 'Leitura de logs e configurações de integração',
  },
  {
    id: 'BG002',
    requestedBy: 'Admin Maria',
    reason: 'Verificar configuração de faturamento após reclamação',
    status: 'expired',
    requestedAt: '2026-05-04 09:00',
    approvedBy: 'Admin Carlos',
    expiresAt: '2026-05-04 11:00',
    scope: 'Leitura de dados financeiros do tenant',
  },
  {
    id: 'BG003',
    requestedBy: 'Admin Carlos',
    reason: 'Diagnóstico de performance — usuário reportou lentidão',
    status: 'pending',
    requestedAt: '2026-05-08 08:00',
    scope: 'Leitura de métricas de uso e logs de API',
  },
];
...
function TenantStatusBadge({ status }: { status: TenantDetail['status'] }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 rounded-full border text-xs font-medium px-2.5 py-1 ${c.classes}`}
    >
      <IconComp size={11} />
      {c.label}
    </span>
...
function PlanBadge({ plan }: { plan: TenantDetail['plan'] }) {
  ...
  return (
    <span
      className={`inline-flex items-center rounded-full border text-xs font-semibold px-2.5 py-1 ${c.classes}`}
    >
      {c.label}
    </span>
  );
...
function SeverityBadge({ severity }: { severity: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center rounded-full border text-xs font-medium px-2 py-0.5 ${config[severity] ?? 'bg-slate-100 text-slate-600 border-slate-200'}`}
    >
...
const featureFlagMeta: {
  key: keyof TenantDetail['featureFlags'];
  label: string;
  icon: React.ElementType;
}[] = [
...
export default function TenantDetailContent() {
  ...
  const tabs = [
    ...
    { key: 'audit', label: 'Auditoria', icon: ClipboardList, count: mockAuditLogs.length },
    {
      ...
      icon: Webhook,
      count: mockWebhookErrors.filter((w) => w.status !== 'resolved').length,
    },
    {
      key: 'support',
      label: 'Suporte',
      icon: Headphones,
      count: mockSupportSessions.filter((s) => s.status === 'open').length,
    },
    {
      key: 'breakglass',
      label: 'Break-Glass',
        ...
        <main className="flex-1 overflow-y-auto scrollbar-thin">
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground ml-14">
                    {tenant.owner} · {tenant.email} · Criado em{' '}
                ...
                return (
                  ...
                  >
                    ...
                    {'count' in tab && tab.count !== undefined && tab.count > 0 && (
                      <span
                        className={`text-xs font-bold rounded-full px-1.5 py-0.5 min-w-[18px] text-center ${isActive ? 'bg-primary text-white' : 'bg-muted text-muted-foreground'}`}
                      >
                        {tab.count}
                      </span>
                    )}
          ...
          <div className="p-6 space-y-5">
              ...
              <>
                    ...
                    <div className="space-y-3">
                      {[
                        ...
                        },
                        {
                          ...
                          value: new Date(tenant.lastActivityAt).toLocaleDateString('pt-BR'),
                        },
                      ...
                      {tenant.saasSubscriptionStatus === 'past_due' && (
                        <div className="flex items-start gap-2 p-2.5 bg-red-50 rounded-lg border border-red-100">
                          ...
                          <p className="text-xs text-red-700">
                            Fatura vencida. Acesso pode ser suspenso em breve.
                          </p>
                        </div>
                      )}
                    </div>
                        ...
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <IntegrationStatusBadge
                            ...
                            label={
                              ...
                              }[tenant.asaasSubaccountStatus] ?? tenant.asaasSubaccountStatus
                            }
                          />
                        </div>
                        <p className="text-xs text-muted-foreground">
                          Subconta:{' '}
                          <span className="font-mono text-foreground">{tenant.asaasAccountId}</span>
                        </p>
                        {tenant.asaasSubaccountStatus === 'blocked' && (
                          <p className="text-xs text-red-600 mt-1 flex items-center gap-1">
                            <AlertCircle size={11} /> Subconta bloqueada — verificar API key
                          </p>
                        )}
                      </div>
                      {/* D4Sign */}
                            ...
                            label={
                              {
                                ...
                                error: 'Erro',
                                not_configured: 'Não configurado',
                              }[tenant.d4signStatus] ?? tenant.d4signStatus
                            }
                          />
                        </div>
                        <UsageBar
                          used={tenant.d4signDocsUsed}
                          capacity={tenant.d4signDocsLimit}
                          unit=" docs"
                        />
                        {tenant.d4signStatus === 'quota_exceeded' && (
                          <p className="text-xs text-orange-600 mt-1 flex items-center gap-1">
                            <AlertCircle size={11} /> Cota mensal esgotada
                          </p>
                        )}
                      </div>
                    </div>
                ...
                <SectionCard title="Métricas de Uso" icon={Activity}>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-5">
                    ...
                    <div>
                      <p className="text-xs text-muted-foreground mb-2 flex items-center gap-1">
                        <HardDrive size={11} /> Armazenamento
                      </p>
                      <UsageBar
                        used={parseFloat(tenant.storageUsedGb.toFixed(1))}
                    ...
                    <div>
                      <p className="text-xs text-muted-foreground mb-2 flex items-center gap-1">
                        <Zap size={11} /> Chamadas de API
                      </p>
                      <UsageBar
                        used={Math.round(tenant.apiCallsThisMonth / 1000)}
                        capacity={Math.round(tenant.apiLimitMonthly / 1000)}
                        unit="k"
                      />
                    </div>
                    <div>
                      <p className="text-xs text-muted-foreground mb-2 flex items-center gap-1">
                        <Calendar size={11} /> Agendamentos (mês)
                      </p>
                      <div className="flex flex-col gap-1">
                        ...
                        <p className="text-xs text-muted-foreground">
                          {tenant.patients} pacientes cadastrados
                        </p>
                      </div>
                    </div>
                  </div>
                      ...
                      return (
                        ...
                        >
                          {React.createElement(FlagIcon, {
                            size: 14,
                            className: enabled ? 'text-teal-600' : 'text-muted-foreground',
                          })}
                          <span
                            className={`text-xs font-medium ${enabled ? 'text-teal-700' : 'text-muted-foreground'}`}
                          >
                            {label}
                          </span>
              ...
              <SectionCard
                ...
                action={
                  <span className="text-xs text-muted-foreground">
                    {mockUsers.length} de {tenant.usersLimit} usuários
                  </span>
                }
              >
                    ...
                    <tbody className="divide-y divide-border">
                      {mockUsers.map((u) => (
                        <tr key={u.id} className="hover:bg-muted/40 transition-colors">
                          ...
                          <td className="py-2.5 px-3">
                            <span
                              className={`inline-flex items-center gap-1 rounded-full border text-xs font-medium px-2 py-0.5 ${u.status === 'active' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-slate-100 text-slate-500 border-slate-200'}`}
                            >
                  ...
                  {mockUnits.map((unit) => (
                    ...
                    >
                      <div className="flex items-start justify-between mb-3">
                        ...
                        <span
                          className={`inline-flex items-center rounded-full border text-xs font-medium px-2 py-0.5 ${unit.status === 'active' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-slate-100 text-slate-500 border-slate-200'}`}
                        >
                      ...
                      <div className="flex items-center gap-4 text-xs text-muted-foreground">
                        <span className="flex items-center gap-1">
                          <Users size={11} /> {unit.users} usuários
                        </span>
                        <span className="flex items-center gap-1">
              ...
              >
                <div className="space-y-1">
                  {mockAuditLogs.map((entry) => {
                    const catConfig = auditCategoryConfig[entry.category] ?? {
                      icon: Info,
                    ...
                    >
                      <div className="flex items-start justify-between gap-3 mb-2">
                        ...
                        <div className="flex items-center gap-2 flex-shrink-0">
                          ...
                          >
                            {
                              { urgente: 'Urgente', alto: 'Alto', medio: 'Médio', baixo: 'Baixo' }[
                                session.priority
                              ]
                            }
                          </span>
                      ...
                      <p className="text-xs text-muted-foreground">
                        {session.assignedTo
                          ? `Atribuído a: ${session.assignedTo}`
                          : 'Não atribuído'}
                      </p>
                    </div>
                  ))}
            ...
            {activeTab === 'breakglass' && (
              <SectionCard
                title="Solicitações Break-Glass"
                icon={Key}
                action={
                  <button className="btn-primary text-xs flex items-center gap-1.5 px-3 py-1.5">
              ...
              >
                <div className="mb-4 flex items-start gap-2.5 p-3 bg-amber-50 rounded-xl border border-amber-200">
                  ...
                  <p className="text-xs text-amber-800">
                    Solicitações break-glass concedem acesso temporário e auditado ao tenant. Todo
                    acesso é registrado e requer aprovação de outro administrador.
                  </p>
                </div>
                <div className="space-y-3">
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-start justify-between gap-3 mb-3">
                          <div className="flex items-center gap-2">
                            <div
                              className={`w-7 h-7 rounded-lg flex items-center justify-center ${bg.status === 'approved' ? 'bg-emerald-50' : bg.status === 'pending' ? 'bg-amber-50' : 'bg-slate-100'}`}
                            >
                              {bg.status === 'approved' ? (
                            ...
                            <div>
                              <p className="text-xs font-semibold text-foreground">
                                Solicitado por {bg.requestedBy}
                              </p>
                              <p className="text-xs text-muted-foreground">{bg.requestedAt}</p>
                            </div>
                          </div>
                          <span
                            className={`inline-flex items-center rounded-full border text-xs font-medium px-2 py-0.5 ${statusCfg.classes}`}
                          >
                            {statusCfg.label}
                          </span>
                        </div>
