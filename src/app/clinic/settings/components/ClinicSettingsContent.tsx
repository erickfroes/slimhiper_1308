...
import { Building2, MapPin, Users, ShieldCheck, Palette, Globe, Plug, CreditCard, BookOpen, Camera, Plus, Edit2, Trash2, Check, ChevronRight, Phone, Save, Upload,  } from 'lucide-react';
...
type PermissionKey =
  | 'pacientes' | 'agenda' | 'atendimento' | 'soap' | 'nutricao' |'prescricoes' | 'documentos' | 'financeiro' | 'pacotes' | 'chat'
...
const mockUnits: ClinicUnit[] = [
  { id: 'u1', name: 'Unidade Central', address: 'Av. Paulista, 1000 – Sala 301', city: 'São Paulo, SP', phone: '(11) 3000-1234', isMain: true, active: true },
  { id: 'u2', name: 'Unidade Moema', address: 'Rua Iraí, 220 – Sala 12', city: 'São Paulo, SP', phone: '(11) 3000-5678', isMain: false, active: true },
  { id: 'u3', name: 'Unidade ABC', address: 'Av. Industrial, 500', city: 'Santo André, SP', phone: '(11) 4000-9012', isMain: false, active: false },
];

const mockTeam: TeamMember[] = [
  { id: 't1', name: 'Dra. Fernanda Lima', role: 'Médica', email: 'fernanda@slimhiper.com', specialty: 'Endocrinologia', active: true, avatarInitials: 'FL', color: 'bg-teal-100 text-teal-700' },
  { id: 't2', name: 'Nutr. Carlos Mendes', role: 'Nutricionista', email: 'carlos@slimhiper.com', specialty: 'Nutrição Clínica', active: true, avatarInitials: 'CM', color: 'bg-violet-100 text-violet-700' },
  { id: 't3', name: 'Ana Souza', role: 'Coordenadora', email: 'ana@slimhiper.com', active: true, avatarInitials: 'AS', color: 'bg-blue-100 text-blue-700' },
  { id: 't4', name: 'Beatriz Costa', role: 'Recepcionista', email: 'beatriz@slimhiper.com', active: true, avatarInitials: 'BC', color: 'bg-amber-100 text-amber-700' },
  { id: 't5', name: 'Dr. Rafael Torres', role: 'Médico', email: 'rafael@slimhiper.com', specialty: 'Nutrologia', active: false, avatarInitials: 'RT', color: 'bg-rose-100 text-rose-700' },
];

const mockRoles: ClinicRole[] = [
  { id: 'admin',         name: 'admin',         label: 'Administrador',  membersCount: 1, color: 'bg-rose-50 text-rose-700 border-rose-200' },
  { id: 'physician',     name: 'physician',     label: 'Médico',         membersCount: 2, color: 'bg-teal-50 text-teal-700 border-teal-200' },
  { id: 'nutritionist',  name: 'nutritionist',  label: 'Nutricionista',  membersCount: 1, color: 'bg-violet-50 text-violet-700 border-violet-200' },
  { id: 'coordinator',   name: 'coordinator',   label: 'Coordenador',    membersCount: 1, color: 'bg-blue-50 text-blue-700 border-blue-200' },
  { id: 'receptionist',  name: 'receptionist',  label: 'Recepcionista',  membersCount: 1, color: 'bg-amber-50 text-amber-700 border-amber-200' },
];

const defaultPermissions = (roleId: string): Record<PermissionKey, { read: boolean; write: boolean }> => {
...
const mockIntegrations: Integration[] = [
  { id: 'i1', name: 'WhatsApp Business', description: 'Envio de mensagens e notificações automáticas', category: 'Comunicação', connected: true, icon: '💬' },
  { id: 'i2', name: 'Google Agenda', description: 'Sincronização bidirecional de consultas', category: 'Agenda', connected: true, icon: '📅' },
  { id: 'i3', name: 'Stripe', description: 'Processamento de pagamentos e assinaturas', category: 'Financeiro', connected: false, icon: '💳' },
  { id: 'i4', name: 'DocuSign', description: 'Assinatura eletrônica de documentos', category: 'Documentos', connected: false, icon: '✍️' },
  { id: 'i5', name: 'Receita Federal', description: 'Validação de CPF e dados cadastrais', category: 'Compliance', connected: true, icon: '🏛️' },
  { id: 'i6', name: 'iClinic', description: 'Importação de prontuários legados', category: 'Migração', connected: false, icon: '🔄' },
];
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <div className="w-20 h-20 rounded-2xl bg-primary/10 flex items-center justify-center border-2 border-dashed border-primary/30 cursor-pointer hover:bg-primary/15 transition-colors group relative overflow-hidden">
          <Camera size={22} className="text-primary/60 group-hover:text-primary transition-colors" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          <label className="block text-xs font-medium text-muted-foreground mb-1.5">E-mail principal</label>
          <input defaultValue="contato@slimhiper.com" type="email" className="input-base w-full text-sm" />
        </div>
        <div>
        ...
        <div>
          <label className="block text-xs font-medium text-muted-foreground mb-1.5">Fuso horário</label>
        ...
        <div className="md:col-span-2">
          <label className="block text-xs font-medium text-muted-foreground mb-1.5">Especialidades</label>
          <input defaultValue="Endocrinologia, Nutrologia, Nutrição Clínica" className="input-base w-full text-sm" />
        </div>
      </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        {mockUnits.map((unit) => (
          <div key={unit.id} className={`bg-background border border-border rounded-xl p-4 ${!unit.active ? 'opacity-60' : ''}`}>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        {mockTeam.map((member) => (
          <div key={member.id} className={`flex items-center gap-3 p-3 bg-background border border-border rounded-xl ${!member.active ? 'opacity-60' : ''}`}>
            <div className={`w-9 h-9 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 ${member.color}`}>
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                ...
                {!member.active && (
                  <span className="text-xs bg-muted text-muted-foreground px-1.5 py-0.5 rounded-full">Inativo</span>
                )}
              </div>
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap gap-2">
        {mockRoles.map((role) => (
          ...
          >
            {role.label}
            <span className={`text-xs px-1.5 py-0.5 rounded-full ${selectedRole === role.id ? 'bg-white/20 text-white' : 'bg-muted text-muted-foreground'}`}>
              {role.membersCount}
            </span>
          </button>
        ))}
      ...
      <div className="border border-border rounded-2xl overflow-hidden">
        ...
        <div className="grid grid-cols-[1fr_80px_80px] bg-muted/60 border-b border-border px-4 py-2.5">
          <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Módulo</span>
          <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wide text-center">Leitura</span>
          <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wide text-center">Escrita</span>
        </div>
      ...
      <div className="flex items-center justify-between">
        <p className="text-xs text-muted-foreground">
          Editando permissões de: <span className="font-semibold text-foreground">{currentRole.label}</span>
        </p>
        <button className="flex items-center gap-2 px-4 py-2 bg-primary text-primary-foreground rounded-xl text-sm font-medium hover:bg-primary/90 transition-colors">
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          <label className="block text-xs font-medium text-muted-foreground mb-2">Cor primária</label>
        ...
        <div>
          <label className="block text-xs font-medium text-muted-foreground mb-2">Cor de destaque</label>
          <div className="flex items-center gap-3">
            <input
              type="color"
              value={accentColor}
      ...
      <div>
        <label className="block text-xs font-medium text-muted-foreground mb-2">Fonte principal</label>
      ...
      <div>
        <label className="block text-xs font-medium text-muted-foreground mb-2">Favicon / ícone do app</label>
        <div className="flex items-center gap-3">
...
function SectionPortal() {
  ...
  const items = [
    { key: 'selfScheduling' as const,  label: 'Auto-agendamento',               desc: 'Pacientes podem agendar consultas pelo app' },
    { key: 'chatEnabled' as const,     label: 'Chat com equipe',                 desc: 'Mensagens diretas entre paciente e profissional' },
    { key: 'documentsAccess' as const, label: 'Acesso a documentos',             desc: 'Visualização de laudos, receitas e resultados' },
    { key: 'financialAccess' as const, label: 'Extrato financeiro',              desc: 'Paciente visualiza cobranças e pagamentos' },
    { key: 'checkInReminder' as const, label: 'Lembretes de check-in',           desc: 'Notificações push para check-ins pendentes' },
    { key: 'npsEnabled' as const,      label: 'Pesquisa de satisfação (NPS)',    desc: 'Envio automático após consultas' },
  ];

  return (
    <div className="space-y-4">
      <div>
        <label className="block text-xs font-medium text-muted-foreground mb-1.5">URL do portal</label>
        <div className="flex items-center gap-2">
          <input defaultValue="https://app.slimhiper.com.br/p/slimhiper" readOnly className="input-base flex-1 text-sm bg-muted/40 text-muted-foreground" />
      ...
      <div className="space-y-2 mt-2">
        {items.map((item) => (
          <div key={item.key} className="flex items-center justify-between p-3.5 bg-background border border-border rounded-xl">
            <div>
        ...
        <div key={cat}>
          ...
          <div className="space-y-2">
            {integrations.filter((i) => i.category === cat).map((intg) => (
              <div key={intg.id} className="flex items-center gap-3 p-3.5 bg-background border border-border rounded-xl">
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          <label className="block text-xs font-medium text-muted-foreground mb-1.5">Dia de vencimento padrão</label>
          <select className="input-base w-full text-sm">
        ...
        <div>
          <label className="block text-xs font-medium text-muted-foreground mb-1.5">Multa por atraso (%)</label>
          <input defaultValue="2" type="number" className="input-base w-full text-sm" />
        </div>
        <div>
          <label className="block text-xs font-medium text-muted-foreground mb-1.5">Juros mensais (%)</label>
      ...
      <div className="space-y-2">
        {[
          { label: 'Emitir NF-e automaticamente', desc: 'Nota fiscal gerada após confirmação de pagamento' },
          ...
          <div key={item.label} className="flex items-center justify-between p-3.5 bg-background border border-border rounded-xl">
            <div>
              <p className="text-sm font-medium text-foreground">{item.label}</p>
              <p className="text-xs text-muted-foreground mt-0.5">{item.desc}</p>
            </div>
...
function SectionProgramas() {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        {programs.map((prog) => (
          <div key={prog.id} className="flex items-center justify-between p-3.5 bg-background border border-border rounded-xl">
            <div className="flex items-center gap-3">
              <div className={`w-2 h-2 rounded-full flex-shrink-0 ${prog.active ? 'bg-primary' : 'bg-muted-foreground/40'}`} />
          ...
          {SECTIONS.map((section) => {
            ...
            return (
              ...
              >
                ...
                <span className={`text-xs ${isActive ? 'font-semibold' : 'font-medium'}`}>{section.label}</span>
      ...
      <div className="flex-1 min-w-0 px-4 lg:px-8 py-6 space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-0.5">Gerencie perfil, equipe, permissões e preferências da clínica</p>
          </div>
        </div>
