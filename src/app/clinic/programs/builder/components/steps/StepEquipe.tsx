...
export default function StepEquipe({ draft, onChange }: Props) {
  ...
  const toggleMember = (member: ProgramBuilderTeamMember) => {
    ...
    onChange({
      team: isSelected
        ? draft.team.filter((m) => m.id !== member.id)
        : [...draft.team, member],
    });
  };
    ...
    <div className="space-y-5">
        ...
        {mockBuilderTeamMembers.map((member) => {
          ...
          return (
            ...
            >
              ...
              <span className={['text-xs font-semibold px-2.5 py-1 rounded-full', roleColor].join(' ')}>
                {member.role}
              </span>
              <div className={[
      ...
      {draft.team.length === 0 && (
        <div className="flex flex-col items-center gap-2 py-6 text-center">
          ...
          <p className="text-sm text-muted-foreground">Selecione os profissionais responsáveis pelo programa</p>
        </div>
      )}
    </div>
  );
