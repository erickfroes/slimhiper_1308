  ...
  return (
    <div className="card p-5 space-y-3">
      <div className="flex items-center gap-3">
        <div className={['w-8 h-8 rounded-lg flex items-center justify-center', isComplete ? 'bg-emerald-50' : 'bg-amber-50'].join(' ')}>
  ...
  return (
    <div className="space-y-5">
      ...
      <div className={[
        ...
        <div>
          <p className={['text-sm font-bold', allComplete ? 'text-emerald-700' : 'text-amber-700'].join(' ')}>
            {allComplete ? 'Programa pronto para publicar!' : `${completedCount} de ${totalChecks} etapas completas`}
          </p>
        ...
        <div className="ml-auto text-right">
          <p className={['text-2xl font-bold', allComplete ? 'text-emerald-600' : 'text-amber-600'].join(' ')}>
      ...
      <ReviewSection icon={BookOpen} title="Dados gerais" isComplete={checks.dadosGerais}>
        {draft.name ? <p><strong>Nome:</strong> {draft.name}</p> : <p className="text-amber-600">Nome não preenchido</p>}
        {draft.programType ? <p><strong>Tipo:</strong> {programTypeLabel[draft.programType]}</p> : <p className="text-amber-600">Tipo não selecionado</p>}
        <p><strong>Duração:</strong> {draft.durationWeeks} semanas · Status: {draft.status}</p>
      </ReviewSection>

      <ReviewSection icon={Layers} title="Fases" isComplete={checks.fases}>
        {draft.phases.length > 0
          ? draft.phases.map((p, i) => (
              <p key={i}><strong>Fase {i + 1}:</strong> {p.name} — {p.durationWeeks} semanas</p>
            ))
      ...
      <ReviewSection icon={Wrench} title="Serviços incluídos" isComplete={checks.servicos}>
        {draft.includedServices.length > 0
          ? draft.includedServices.map((s, i) => (
              <p key={i}>{s.quantity}x {s.label}</p>
            ))
      ...
      <ReviewSection icon={Smartphone} title="Entitlements do app" isComplete={checks.entitlements}>
        {draft.appEntitlements.filter((e) => e.enabled).map((e) => (
      ...
      <ReviewSection icon={CheckSquare} title="Check-ins" isComplete={checks.checkins}>
        <p><strong>Total:</strong> {draft.checkInsTotal} check-ins · {draft.checkInFrequency}</p>
        {draft.checkinTemplates.length > 0 && (
          <p><strong>Templates:</strong> {draft.checkinTemplates.map((t) => t.label).join(', ')}</p>
        )}
      </ReviewSection>
      ...
      <ReviewSection icon={CreditCard} title="Financeiro" isComplete={checks.financeiro}>
        <p><strong>Modelo:</strong> {paymentModelLabel[draft.financial.paymentModel]}</p>
        <p><strong>Valor base:</strong> R$ {draft.financial.basePrice.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>
        {draft.financial.discountPercent ? (
          <p><strong>Desconto à vista:</strong> {draft.financial.discountPercent}%</p>
        ) : null}
