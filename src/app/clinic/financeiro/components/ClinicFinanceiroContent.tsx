...
export default function ClinicFinanceiroContent() {
  const [data, setData] =
    useState<Awaited<ReturnType<typeof getClinicFinanceOverview>>['data']>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    void (async () => {
      const r = await getClinicFinanceOverview();
      setData(r.data);
  ...
  return (
    <div className="p-4 lg:p-6 space-y-6">
      ...
      {loading && (
        <section className="bg-card border rounded-2xl p-4 text-sm text-muted-foreground">
          Carregando...
        </section>
      )}
      {error && (
        <section className="bg-card border rounded-2xl p-4 text-sm text-red-600">{error}</section>
      )}
      <section className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
        {metrics.map((m) => (
          <article key={m.label} className="bg-card border border-border rounded-2xl p-4">
      ...
      <section className="bg-card border rounded-2xl p-5">
        ...
        <div className="mt-3 space-y-2">
          {(data?.recentCharges ?? []).map((c) => (
            <div key={c.id} className="text-sm border rounded-lg p-2 flex justify-between">
              <span>
                {c.patientName} • {c.description}
              </span>
              <span>{brl(c.amount)}</span>
            </div>
          ))}
        </div>
      </section>
