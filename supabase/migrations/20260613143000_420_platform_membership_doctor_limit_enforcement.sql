-- Enforce plan doctor limits inside the audited platform membership mutator.
-- This closes the backend/RPC gap so direct RPC usage cannot bypass the API preflight.
...
declare
  ...
  v_next_status text;
  v_doctors_limit integer;
  v_current_physicians integer := 0;
...
begin
  ...
  if v_next_role_code = 'physician' and v_next_status in ('active', 'invited') then
    select
      ...
      into v_doctors_limit
    from public.tenants t
    where t.id = p_tenant_id;

    if v_doctors_limit is not null and v_doctors_limit > 0 then
      select count(*)::integer
        into v_current_physicians
      ...
      if v_current_physicians + 1 > v_doctors_limit then
        raise exception 'tenant_doctors_limit_exceeded'
          using errcode = '23514',
                detail = jsonb_build_object(
                  'current', v_current_physicians,
                  'limit', v_doctors_limit
                )::text;
      end if;
    end if;
  end if;
...
comment on function public.update_platform_tenant_membership(uuid, uuid, text, text, uuid, text) is 'Updates an existing tenant membership role/status/unit for platform admins with mandatory reason, audit log and plan doctor limit enforcement.';
