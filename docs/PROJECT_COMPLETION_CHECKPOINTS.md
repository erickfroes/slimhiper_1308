...
| Fase 9 - CRM/estoque               | PR 9.3 implementada                    | Pre-requisitos revisados: Fase 8 possui migrations 180-183 para relatorios/chat/portal/comunicacoes; PR 9.1 entregou `20260601190000_190_crm_inventory_contracts.sql` com fundacao CRM/estoque, RBAC granular, RLS/grants e RPCs auditadas; PR 9.2 entregou CRM operacional com `crmApi`, `/clinic/crm`, detalhe, tarefas, notificacoes e conversao idempotente lead -> paciente. PR 9.3 adiciona `20260601213000_192_inventory_operations.sql` com snapshot operacional de estoque, upsert auditado de item, criacao de lote, ledger com motivo obrigatorio, bloqueio de saldo negativo, transferencia em duas movimentacoes e notificacoes de alertas; `inventoryApi` real/mock-gated e `/clinic/inventory` operacional com filtros, saldos, lotes/validade, cadastro, recebimento/consumo/perda/ajuste/transferencia e custo gated por `inventory.cost.read`. Validacao Supabase local/migration up segue pendente porque nao foi executado `supabase db push` nem provider externo; checks Next/TS/lint/build passaram neste lote. |
