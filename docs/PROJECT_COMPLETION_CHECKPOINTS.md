...
| Fase 9 - CRM/estoque               | PR 9.4 implementada                    | Pre-requisitos revisados e mantidos: Fase 8 possui migrations 180-183 para relatorios/chat/portal/comunicacoes; PR 9.1 entregou fundacao/RBAC/RLS/RPCs de CRM/estoque; PR 9.2 entregou CRM operacional e conversao idempotente lead -> paciente com timeline/audit log; PR 9.3 entregou estoque operacional por lote/local/ledger e alertas. PR 9.4 adiciona `20260601223000_193_crm_inventory_insights.sql` com allowlist de relatorios agregados de CRM/estoque, custo gated por `inventory.cost.read`, export seguro com rechecagem RBAC e RPC `get_crm_inventory_dashboard_insights`; `dashboardApi` e `/clinic/dashboard` exibem cards discretos de leads/tarefas vencidas e estoque/lotes criticos; `/clinic/reports` reconhece icones/badges dos novos relatorios. Nao houve `supabase db push`, provider API ou script mutante; Supabase local/migration up segue pendente por ambiente. Checks `npm run type-check`, `npm run lint`, `npm run build` e `git diff --check` passaram neste lote; lint manteve 13 warnings preexistentes sem erros. |
