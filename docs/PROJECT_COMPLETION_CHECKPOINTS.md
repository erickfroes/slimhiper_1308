...
negado`, e perfil `is_active=false` -> `app-session` `authenticated=false`,
  ...
  Fase 7 implementada aguardando validacao local: migration `20260531203000_150_platform_admin_audit_contracts.sql`
  ...
  `/admin/webhooks` consomem os contratos reais sem fixtures operacionais; a rota
  server-side `/api/admin/tenants/[tenantId]/invitations` usa service role apenas
  no backend para convite Auth Admin, valida tenant/role/unidade, grava perfil,
  membership `invited` e audit log `platform_tenant_membership.invited`; e
  `test-platform-admin-phase7-local-smoke.mjs` cobre o fluxo local autorizado de
  RPCs support/break-glass/membership. Nesta sessao, `npm run type-check`,
  `npm run lint` e `npm run build` passaram; `npx supabase migration up --local
  --include-all`, `npx supabase start` e `test-platform-admin-phase7-local-smoke.mjs`
  ficaram bloqueados porque nao ha Docker/Postgres local nem variaveis Supabase
  disponiveis no ambiente.
- Proximos bloqueios: alteracao real de permissoes por RPC, settings por usuario,
  aplicar/validar as migrations Fase 7 de admin plataforma em ambiente com Docker/Postgres,
  UI/contratos scoped do portal paciente antes de abrir `/patient`,
...
| Fase 4 - Documentos/D4Sign         | Concluida para MVP local/sandbox        | Templates/UI, PDF local, variaveis permitidas, policy paciente/guardian, signed URL, monitor operacional, webhook/idempotencia/auditoria e envio D4Sign sandbox passaram. Hardening adicional de producao em 2026-06-05 remove escrita direta autenticada em `signature_requests`/`signature_signers`; callback provider real ainda deve ser validado em homologacao se o go-live exigir evidência externa completa.                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| Fase 5 - Financeiro/Asaas          | Concluida para MVP local/sandbox        | RPCs/Edge/webhook/fixtures locais, conciliacao/divergencias e Asaas sandbox estrito passaram; customer/invoice/subscription continuam gated por Edge Functions, JWT no gateway, `financial.write` e service-role backend apos autorizacao.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| Fase 9 - CRM/estoque               | PR 9.5 validada localmente              | Pre-requisitos revisados e mantidos: Fase 8 possui migrations 180-183 para relatorios/chat/portal/comunicacoes; PRs 9.1-9.4 entregaram fundacao RBAC/RLS/RPCs, CRM operacional com conversao idempotente, estoque ledger por lote/local e insights agregados em dashboard/relatorios. PR 9.5 adiciona `20260601233000_195_crm_inventory_governance_hardening.sql` com snapshot agregado de governanca e helper service-role de retencao dry-run/execucao explicita, `test-crm-inventory-phase9-local-smoke.mjs` com tenants A/B, permissoes negativas, deduplicacao, conversao, ledger, saldo negativo, auditoria, notificacoes e reports, e runbook `CRM_INVENTORY_GOVERNANCE_RUNBOOK.md` para LGPD/operacao. Em 2026-06-05 o smoke local/Docker passou sem provider API ou chamada externa. |
| Fase 10 - Producao/observabilidade | Hardening local validado                | Pre-requisitos revisados: PRs 10.1-10.5 seguem aplicadas. Em 2026-06-05 a migration `200_provider_production_hardening` foi aplicada no Supabase local/Docker, os smokes locais Fases 1-9 e pos-deploy passaram, webhook Asaas sintetico validou fail-closed/idempotencia, D4Sign HMAC local permaneceu aprovado, e restore schema-only local passou em banco temporario. Go-live permanece NO-GO ate aplicar em staging/producao pela esteira oficial, configurar metadata/sink de alerta, rodar browser autenticado em staging e obter aprovacao humana LGPD/security/juridica.                           |
...
- [x] Webhooks D4Sign/Asaas sao fail-closed, idempotentes e nao logam payload sensivel. Parcial de release: callbacks provider reais ainda precisam evidencia em homologacao se forem gate de go-live.
- [x] Documentos privados usam signed URL curta gerada server-side ou Edge Function, nunca URL publica direta. Em 2026-06-05 CORS do `document-signed-url` passou a usar allowlist/local-only por origem.
...
- [x] Contratos reais/sandbox so rodam com autorizacao explicita e ambiente segregado. Asaas e D4Sign sandbox passaram em ambiente autorizado; callbacks provider externos ainda sao evidencia complementar de homologacao.
...
- [x] Persistir settings clinicas e admin tenants/users/units. Settings clinicas de tenant/unidade persistem localmente por RPC auditada; admin tenants/users/units leem contratos reais por `list_platform_tenants` e `get_platform_tenant_detail`; `update_platform_tenant_membership` altera role/status/unidade de usuario existente via RPC auditada com motivo obrigatorio; convites Auth Admin usam rota server-side com service role backend, validacao de tenant/role/unidade, membership `invited` e audit log.
- [x] Fechar admin shell e guards de plataforma. `AdminShell` compartilhado removeu sidebars/topbars duplicadas em admin overview, tenants, detalhe e webhooks; rotas seguem sob guard server-side de plataforma existente; build confirma rotas admin e API de convites. Browser autenticado amplo ainda depende de ambiente local com Supabase.
- [x] Implementar support/break-glass com auditoria forte. `request_platform_support_session`, `end_platform_support_session`, `request_platform_break_glass`, `decide_platform_break_glass` e `revoke_platform_break_glass` exigem platform admin/support, justificativa, duracao limitada, bloqueio de autoaprovacao, encerramento/revogacao operacional e audit log; smoke local existe e ficou bloqueado nesta sessao por falta de Docker/Postgres/env Supabase.
- [x] Criar eventos auditaveis para fluxos sensiveis. Support/break-glass, revogacao/encerramento operacional, alteracao de membership e convite Auth Admin registram `audit_logs`; webhooks/admin exibem resumos sanitizados. Login sensivel, troca de tenant e exportacoes ficam como gates de producao/Fase 10.
...
- [x] Criar retencao e moderacao para comunicacoes. Evidencia PR 8.4: migration `20260601163000_183_communications_governance.sql` adiciona `retention_until`, `archived_at`, `moderation_status`, `moderation_reason`, `moderated_by`, `moderated_at`, `read_receipts`, tabela `communication_governance_events`, mutators auditados de moderacao e RPC service-role de expiracao; inbox/header/Paciente 360/portal ocultam conteudo moderado com placeholders fail-closed; runbook `COMMUNICATIONS_GOVERNANCE_RUNBOOK.md` define prazos LGPD e `check-communications-retention.mjs` faz dry-run agregado sem mutacao.
...
_Status PR 8.4: implementado nesta branch. A governanca adiciona campos/tabela de retencao e moderacao, placeholders fail-closed em surfaces clinicas/portal, runbook LGPD e script dry-run read-only. Smokes Supabase/browser dependem de ambiente local com Docker/Postgres e variaveis Supabase._
...
_Status PR 9.5: implementado nesta branch. A migration `20260601233000_195_crm_inventory_governance_hardening.sql` adiciona snapshot agregado de governanca e helper service-role de retencao de leads nao convertidos com dry-run por padrao; `test-crm-inventory-phase9-local-smoke.mjs` prepara fixtures dummy deterministicas e cobre tenants A/B, permissoes negativas, deduplicacao, conversao idempotente, ledger, bloqueio de saldo negativo, auditoria, notificacoes e relatorios; `CRM_INVENTORY_GOVERNANCE_RUNBOOK.md` documenta LGPD/opt-out/anonimizacao/anexos/inventario fisico/perda/recall. Smokes Supabase/browser dependem de ambiente local com Docker/Postgres e variaveis Supabase._
...
_Status PR 10.2: implementado nesta branch. Os pre-requisitos da PR 10.1 foram
mantidos e os gates pendentes de Fases 1-9 continuam bloqueadores de release,
nao autorizacao de producao. A app agora expoe `/api/health` com status seguro
por ambiente e headers de correlacao; `/api/auth/app-session` emite logs JSON
redigidos para falhas/sucessos de sessao; `webhook-d4sign` e `webhook-asaas`
propagam request/correlation id e registram eventos estruturados para signature
failures, duplicidade/idempotencia, processamento e erros internos sem payloads
brutos. `/admin/observability` entrega o dashboard operacional por ambiente;
`scripts/observability/post-deploy-smoke.mjs` valida health, login e redirects
fail-closed em modo read-only; `docs/operations/OBSERVABILITY_ALERTING_RUNBOOK.md`
define metricas, alertas S1-S4, canais, owners, ack, escalonamento, smoke e
teste de alerta controlado; os templates de ambiente ganharam variaveis de
observabilidade sem valores reais. Nao foram chamadas APIs de providers, nao
houve `supabase db push`, migrations, retries de webhook, restore, rotacao de
chaves, leitura de `.env` ou promocao de ambiente._
...
- [x] `feat/crm-inventory-contracts`
- [x] `feat/crm-leads-pipeline`
