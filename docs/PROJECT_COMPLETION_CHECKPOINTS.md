...
| Fase 9 - CRM/estoque               | PR 9.1 implementada                    | Pre-requisitos revisados: Fase 8 possui migrations 180-183 para relatorios/chat/portal/comunicacoes; inventario tecnico encontrou apenas tipos/eventos mock de lead no Patient 360 e nenhum modulo operacional de CRM/estoque; a primeira entrega usa `inventory_locations` independentes com vinculo opcional a `tenant_units`. Migration `20260601190000_190_crm_inventory_contracts.sql` adiciona fundacao CRM/estoque, permissoes RBAC granulares, RLS/grants, RPCs auditadas de contrato minimo e ledger de estoque com bloqueio de saldo negativo. Validacao Supabase local segue pendente em ambiente com Docker/Postgres. |
