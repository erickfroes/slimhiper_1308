...
publicas, todas com RLS habilitado, e 85 funcoes publicas. O token carregado
estava no formato versionado `sbp_v0_...`; para o Supabase CLI 2.104.0, ele foi
normalizado em memoria para o formato legado esperado, sem alterar `.env` nem
imprimir valor. Com isso, a Edge Function alterada
`asaas-create-tenant-subaccount` tambem foi publicada no projeto remoto. A
previsao operacional atual e: 1-2 dias uteis para bootstraps/smokes autenticados
sem mock; 3-5 dias uteis para fechar evidencias de release se D4Sign/Asaas
sandbox e browser autenticado tambem estiverem disponiveis.
...
--yes`; o dry-run listou 29 migrations pendentes, de `20260530120000` a
  ...
  11. Identificado que o token estava no formato versionado `sbp_v0_...`; o
      valor foi normalizado em memoria para o formato legado aceito pelo CLI
      2.104.0, sem alterar `.env` e sem imprimir o token.
  12. Executado deploy remoto com
      `functions deploy asaas-create-tenant-subaccount --project-ref <redacted>
--use-api`; a funcao e o helper `_shared/env.ts` foram publicados no
      projeto remoto SlimHiper.
...
- Resultado observado: migrations remotas aplicadas e confirmadas; correcao de
  codigo da Edge Function concluida e publicada no projeto remoto.
...
- Status: migrations remotas aprovadas; Edge Function F11 publicada; validacao
  Asaas sandbox pendente.
- Pendencias: executar bootstrap/smokes autorizados, validar Asaas sandbox sem
  expor valores e executar browser smoke autenticado.
...
### Evidencia - fechamento funcional por contratos de producao

- Data: 2026-06-05.
- Ambiente: implementacao local com `.env` atual, sem imprimir valores de
  ambiente e sem alterar o arquivo `copia.env`/`cópia.env`.
- Implementado nesta rodada:
  1. Nova migration `20260605183000_210_complete_functionality_contracts.sql`
     fecha contratos de prescricoes, recibos, negociacoes, lembretes,
     contratos financeiros, evidencias documentais, reprocessamento de webhook,
     alteracao de papel clinico, flags de revisao e status de pacote.
  2. Paciente 360 ganhou acoes mutaveis reais nas abas de consultas,
     prescricoes, nutricao, pacotes, documentos e financeiro, preservando
     fallback mock somente quando `NEXT_PUBLIC_USE_MOCK_DATA=true`.
  3. Configuracoes da clinica ganharam edicao auditada de papel e convite de
     membro por rota server-side com Supabase Auth Admin, validacao de tenant,
     unidade, papel e motivo auditavel.
  4. Admin webhooks ganhou solicitacao de reprocessamento auditavel; billing
     passou a suportar recibo, pagamento manual, contrato, negociacao e
     lembrete local; documentos passaram a expor evidencias por paciente.
  5. `patient-360-summary` passou a retornar metadados de prescricao ligados a
     documentos gerados para habilitar fluxo de assinatura quando permitido.
- Validacoes executadas nesta rodada:
  1. `npm run type-check` passou apos as novas telas/servicos.
  2. A bateria final de lint, build, `git diff --check`, fixtures
     nao-provider e browser smoke deve ser repetida antes do commit desta
     rodada.
- Pendencias para producao:
  1. Aplicar a migration `210` no ambiente de homologacao/producao pela esteira
     oficial e somente depois publicar Edge Functions que dependem das novas
     colunas/RPCs.
  2. Executar browser autenticado em staging com usuarios sinteticos por perfil
     e validar isolamento tenant/unidade, storage e signed URLs.
  3. Testar D4Sign e Asaas em sandbox real com credenciais homologadas e
     evidencias humanas, sem payloads sensiveis em logs.
  4. Configurar metadata de release/observabilidade para remover warnings de
     go-live e formalizar aprovacao LGPD/security/juridica.
